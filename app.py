...
class resume_analyzer:
    ...
    def openai_analysis(openai_api_key, chunks, analyze):
    ...
    def groq_analysis(groq_api_key, chunks, analyze):
        try:
            ...
            prompt = f"""
            Based on the following resume content, please answer the question below:
    ...
    def analyze_resume(api_key, chunks, analyze, api_type="openai"):
        """
        Unified function to analyze resume using either OpenAI or Groq API
        """
        if api_type.lower() == "openai":
            return resume_analyzer.openai_analysis(api_key, chunks, analyze)
        elif api_type.lower() == "groq":
            return resume_analyzer.groq_analysis(api_key, chunks, analyze)
        else:
            return "Invalid API type selected"
            ...
            if pdf is not None and api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        summary = resume_analyzer.analyze_resume(
                            api_key=api_key, 
                            chunks=pdf_chunks, 
                ...
                st.markdown(f'<h5 style="text-align: center;color: orange;">Please Enter API Key</h5>', unsafe_allow_html=True)
            ...
            if pdf is not None and api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        strength = resume_analyzer.analyze_resume(
                            api_key=api_key, 
                            chunks=pdf_chunks, 
            ...
            if pdf is not None and api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        weakness = resume_analyzer.analyze_resume(
                            api_key=api_key, 
                            chunks=pdf_chunks, 
            ...
            if pdf is not None and api_key != '':
                try:
                    with st.spinner('Processing...'):
                        ...
                        job_title = resume_analyzer.analyze_resume(
                            api_key=api_key, 
                            chunks=pdf_chunks, 
